...
namespace system_tray {
  ...
  static struct tray tray = {
    ...
    .menu =
      (struct tray_menu[]) {
        ...
        { .text = "Open LavApollo", .cb = tray_open_ui_cb },
        { .text = "-" },
        // { .text = "-" },
        // { .text = "Donate",
        //   .submenu =
  ...
  #ifdef _WIN32
    std::string tmp_str = "Open LavApollo (" + config::nvhttp.sunshine_name + ":" + std::to_string(net::map_port(confighttp::PORT_HTTPS)) + ")";
    static const std::string title_str = utf8ToAcp(tmp_str);
  #else
    static const std::string title_str = "Open LavApollo (" + config::nvhttp.sunshine_name + ":" + std::to_string(net::map_port(confighttp::PORT_HTTPS)) + ")";
  #endif
